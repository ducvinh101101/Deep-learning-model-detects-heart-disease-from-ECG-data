...
from keras.src.layers import Reshape, LayerNormalization, GlobalAveragePooling1D
...
from keras.api.optimizers import Adam,RMSprop,SGD,Nadam
...
class MultiHeadAttention(Layer):
    def __init__(self, num_heads, embedding_dim, dropout=0.1, **kwargs):
    ...
    def call(self, query, key, value, mask=None):
        ...
        output = tf.reshape(output, (batch_size, -1, self.embedding_dim))
...
attention_layer = MultiHeadAttention(num_heads=8, embedding_dim=64, dropout=0.2)
x_ecg = attention_layer(query=x_ecg, key=x_ecg, value=x_ecg)
x_ecg = Dense(256 , activation='relu', kernel_regularizer=l2(0.001))(x_ecg)
...
x_metadata = GlobalAveragePooling1D()(x_metadata)
